...
# 트레이닝 데이터
df = pd.read_excel('train/train_amore.xlsx')
data2 = np.array(df)
...
dtype = torch.float32
D_in, H, D_out = 9, 500, outputlen


try:
    w1re = pd.read_excel('wbdata/w1_amore.xlsx', index_col=None)
    w1np = np.array(w1re)
    w1ts = torch.Tensor(w1np)

    w2re = pd.read_excel('wbdata/w2_amore.xlsx', index_col=None)
    w2np = np.array(w2re)
    w2ts = torch.Tensor(w2np)

    b1re = pd.read_excel('wbdata/b1_amore.xlsx', index_col=None)
    b1np = np.array(b1re)
    b1ts = torch.Tensor(b1np)

    b2re = pd.read_excel('wbdata/b2_amore.xlsx', index_col=None)
    b2np = np.array(b2re)
...
raw.to_excel(excel_writer='wbdata/w1_amore.xlsx',index=None)
raw = pd.DataFrame(np.array(w2))
raw.to_excel(excel_writer='wbdata/w2_amore.xlsx',index=None)
raw = pd.DataFrame(np.array(b1))
raw.to_excel(excel_writer='wbdata/b1_amore.xlsx',index=None)
raw = pd.DataFrame(np.array(b2))
raw.to_excel(excel_writer='wbdata/b2_amore.xlsx',index=None)


df2 = pd.read_excel('test/test_amore.xlsx')
data3 = np.array(df2)
...
raw.to_excel(excel_writer='bqdata/backquery_amore.xlsx', index=None)
